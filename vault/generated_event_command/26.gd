...
func _get_commands_list():
    ...
    commands[1] = func():
        await push(["push_dialogue", "name_blob", "lv3_extra_31"])
        await push(["push_dialogue", "name_blob", "lv3_extra_32"])
        await push(["push_dialogue", "name_blob", "lv3_extra_33"])
    ...
    commands[2] = func():
        await push(["open_inventory", func(id):
            if id == 12:
                push(["erase_item"])
                await push(["push_dialogue", "name_blob", "lv3_extra_34"])
                await push(["push_dialogue", "name_blob", "lv3_extra_35"])
                await push(["push_dialogue", "name_blob", "lv3_extra_36"])
