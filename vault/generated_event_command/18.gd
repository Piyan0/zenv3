...
func _get_commands_list():
    ...
    commands[1] = func():
        await push(["push_dialogue", "name_blob", "lv3_extra_01"])
        await push(["start_dialogue"])
        
    commands[2] = func():
        await push(["push_dialogue", "name_blob", "lv3_extra_11"])
        await push(["push_dialogue", "name_blob", "lv3_extra_12"])
        await push(["push_dialogue", "name_blob", "lv3_extra_13"])
        await push(["push_dialogue", "name_blob", "lv3_extra_14"])
        await push(["push_dialogue", "chloe", "got_item"])
        await push(["start_dialogue"])
        push(["set_switch", "got_drawing"])
        push(["add_item", 11])
        push(["set_iswitch", "a"])
