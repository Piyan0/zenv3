...
func _get_commands_list():
    ...
    commands[3] = func():
        await push(["open_inventory", func(id):
            if id == 11:
                push(["erase_item"])
                push(["push_dialogue", "name_blob", "lv3_extra_21"])
                push(["push_dialogue", "name_blob", "lv3_extra_22"])
                push(["push_dialogue", "name_blob", "lv3_extra_23"])
                await push(["start_dialogue"])
                await push(["show_image", "img_reds_drawing"])
                push(["push_dialogue", "name_blob", "lv3_extra_24"])
                push(["push_dialogue", "name_blob", "lv3_extra_25"])
